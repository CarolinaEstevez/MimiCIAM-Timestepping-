...
@defcomp slrcost begin
    ...
    R = Variable(index = [time, segments, 6], unit = "m")       # Retreat perimete r (m)
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_first(t)
            ...
            for m in d_segments
                ...
                v.coastArea[t, m] = calcCoastArea(view(v.areaparams, m, :), v.lslr_update[t, m])
                ...
                for i = 2:Int(p.ntsteps)
                    ...
                    v.wetlandloss[tim1, m] = min(1, (localrate(v.lslr_update[tim1, m], v.lslr_update[ti, m], p.tstep) / p.wmaxrate)^2)


                end
                v.wetlandloss[TimestepIndex(p.ntsteps), m] = min(1, (localrate(v.lslr_update[TimestepIndex(p.ntsteps - 1), m], v.lslr_update[TimestepIndex(p.ntsteps), m], p.tstep) / p.wmaxrate)^2)
        ...
        if (gettime(t) in p.at)
            ...
            if at_index_next <= length(p.at)
                ...
                at_next = p.ntsteps # Flag this assumes timesteps are indices not years (1:20 not 2010:2200)
                last_t = at_next
                last = 1
            end
            ...
            for m in d_segments
                if atstep == 0
                    ...
                    for i in t_range
                        ti = TimestepIndex(i)
                        tim1 = TimestepIndex(i - 1)
                        R_NoAdapt = max(0, v.lslr_update[ti, m])
                    ...
                    lslrPlan_at = v.lslr_update[TimestepIndex(at_next), m]
                    lslrPlan_atprev = v.lslr_update[t, m]

                    for i = 1:length(p.adaptoptions)
                        if is_first(t)
                            Rprev = calcHorR(-2, p.adaptoptions[i], v.lslr_update[ti1, m], view(p.surgeexposure, m, :), p.adaptoptions)
                        ...
                        v.SIGMA[t, m, i+1] = (p.rsig0[m] / (1 + p.rsigA[m] * exp(p.rsigB[m] * max(0, v.R[t, m, i] - v.lslr_update[t, m]))))
                        ...
                        if p.adaptoptions[i] >= 10 || p.adaptoptions[i] == 0
                            ...
                            if is_first(t)
                                ...
                                Hprev = calcHorR(-1, p.adaptoptions[i], v.lslr_update[ti1, m], view(p.surgeexposure, m, :), p.adaptoptions)
                                v.H[t, m, i-1] = calcHorR(-1, p.adaptoptions[i], lslrPlan_at, view(p.surgeexposure, m, :), p.adaptoptions)
                                v.SIGMA[t, m, (i-1)+7] = (p.psig0[m] + p.psig0coef[m] * max(0, v.lslr_update[t, m])) / (1.0 + p.psigA[m] * exp(p.psigB[m] * max(0, (v.H[t, m, i-1] - v.lslr_update[t, m]))))
                                ...
                                if p.fixed == false
                                    ...
                                    v.SIGMA[t, m, (i-1)+7] = (p.psig0[m] + p.psig0coef[m] * max(0, v.lslr_update[t, m])) / (1.0 + p.psigA[m] * exp(p.psigB[m] * max(0, (v.H[t, m, i-1] + v.OptimalR[TimestepIndex(gettime(t) - 1), m] - v.lslr_update[t, m]))))
                        ...
                        for j in t_range
                            ...
                            v.SIGMA[tj, m, i+1] = (p.rsig0[m] / (1 + p.rsigA[m] * exp(p.rsigB[m] * max(0, v.R[tj, m, i] - v.lslr_update[tj, m]))))
                            ...
                            if p.adaptoptions[i] >= 10 || p.adaptoptions[i] == 0
                                ...
                                if p.fixed == false && !(is_first(t))
                                    ...
                                    v.SIGMA[tj, m, (i-1)+7] = (p.psig0[m] + p.psig0coef[m] * max(0, p.lslr[tj, m])) / (1.0 + p.psigA[m] * exp(p.psigB[m] * max(0, (v.H[tj, m, i-1] - v.lslr_update[tj, m]))))
                    ...
                    elseif v.OptimalOption[t, m] == -2
                        ...
                        if is_first(t)
                            ...
                            v.OptimalR[t, m] = max(0, v.lslr_update[t, m])

                            for i in t_range
                                ti = TimestepIndex(i)
                                if i > 1
                                    v.OptimalR[ti, m] = max(v.OptimalR[t, m], v.OptimalR[TimestepIndex(i - 1), m], v.lslr_update[ti, m])
                                else
                                    v.OptimalR[ti, m] = max(v.OptimalR[t, m], v.lslr_update[ti, m])
                                end
                                v.WetlandLossOptimal[ti, m] = v.wetlandloss[ti, m] * min(v.coastArea[ti, m], p.wetland[m])
                                if i == 1
                            ...
                            for i in t_range
                                ti = TimestepIndex(i)
                                v.OptimalR[ti, m] = max(v.OptimalR[TimestepIndex(gettime(t) - 1), m], v.OptimalR[TimestepIndex(i - 1), m], v.lslr_update[ti, m])
