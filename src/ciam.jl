...
function initciam(xsc, params, initparams, m::Model; fixed::Bool = true, t::Int = 20,
    ...
    for k in keys(params)
        if k == "lslr_modeled"|| k=="fp"
            params[k]=params[k]
        elseif ndims(params[k]) !== 0 && ndims(params[k]) == 2 && size(params[k])[1] > t && k != "surgeexposure"
    ...
    params_dict = Dict()
    for (k,v) in params
        if k in ["fp","LWS","GLAC","GIS","AIS","TE"]
            params_dict[(:lslr_model, k)] = v
        else
    ...
    #Add lslr_modeled parameter
    connect_param!(m, :slrcost => :lslr_modeled, :lslr_model => :lslr_modeled)
...
function get_model(;initfile::Union{String, Nothing} = nothing, fixed::Bool=true,
                    t::Int = 20, noRetreat::Bool = false, allowMaintain::Bool = false,
                    popinput::Int = 0, GAMSmatch::Bool = false, surgeoption::Int = 0, n::Int = 450)

    initparams  = init(; f = initfile)
    params, xsc = import_model_data(initparams["lslr"][1], initparams["subset"][1],
                                    initparams["ssp"][1], initparams["ssp_simplified"][1], initparams["lsldatafile"][1]
                                    ,initparams["fingerprint"][1], popinput, surgeoption)
    ...
    set_dimension!(m, :nmodelt, n)

    add_comp!(m, lslr_model)
