...
"""
    prepmodeledlsl!(lslfile,subset, params,segnames)

Read data for modeling LSLR from file and filter to desired set of segments, note that this modifies
the input parameter dictionary `params`.  The arguments are as fullows:

- lsldatafile - name of lslr data file to use; location relative to data/input-data directory
- fingerprint - name of fingerprint file to use;  location relative to data/input-data directory
- subset - list of segments you want
- params - parameter dictionary you want to add lslr to
- segnames - names of segments
"""
function prepmodeledlsl!(lsldatafile, fingerprint, subset, params, segnames)

    #data_dir = joinpath(@__DIR__,"..","data","lslr")
    lsl_params = CSV.read(lsldatafile, DataFrame) |> DataFrame
    data_params = convert(Array{Float64,2}, Matrix(lsl_params))

    fp_params = CSV.read(fingerprint, DataFrame) |> DataFrame
    fp_data_params = convert(Array{Float64,2}, Matrix(fp_params))

    params["fp"] = fp_data_params
    params["tmodel"] = collect(11:10:201)
    params["LWS"]= data_params[:,3]
    params["GLAC"]= data_params[:,4]
    params["GIS"]= data_params[:,5]
    params["AIS"]= data_params[:,6]
    params["TE"]= data_params[:,7]
...
"""
function import_model_data(lslfile,sub,ssp,ssp_simplified,lsldatafile, fingerprint,popinput,surgeoption)
    ...
    prepmodeledlsl!(lsldatafile,fingerprint, subset, params,xsc[3])
...
function write_init_file(run_name::String, outputdir::String, init_settings::Dict)
    textheader="run_name,lslr,subset,ssp,ssp_simplified,lsldatafile,fingerprint\n"
    textstr = "$(run_name),$(init_settings[:lslrfile]),$(init_settings[:subset]),$(init_settings[:ssp]),$(init_settings[:ssp_simplified]),$(init_settings[:lsldatafile]),$(init_settings[:fingerprint])"
